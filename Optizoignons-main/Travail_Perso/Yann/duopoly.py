...
def p(
    ...
    if day == 1:
        ...
        information_dump['factor'] = 1.0
        information_dump['last_base_price'] = np.random.uniform(30, 50)
    ...
    if day % 5 == 0:
        # Sécurise l’accès à courbe[day]
        if day in courbe:
            capacity_obj = courbe[day]['cap_util']
        else:
            # si la clé n’existe pas, on prend la dernière connue
            last_key = max(courbe.keys())
            capacity_obj = courbe[last_key]['cap_util']
            print(f"⚠️ Jour {day} non trouvé dans 'courbe' — utilisation de la capacité du jour {last_key}")

        remaining_capacity = 80 - np.sum(demand)
        factor =capacity_obj/ (remaining_capacity / 80) 
    ...
    # --- Sécurité : prix plancher ---
    if price_today < 20:
        price_today = 20

    return price_today, information_dump, demand
